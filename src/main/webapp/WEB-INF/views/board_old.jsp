...
<script>
    $(document).ready(function () { //main() 브라우저가 html코드를 다 읽고나면 여기 이벤트가 발생
        $('#listBtn').on("click",function (){ //id가 listBtn인 요소를 참조하겠다는 뜻
            location.href = "<c:url value='/board/list'/>?page=${page}&pageSize=${pageSize}";
        });
    });
    $(document).ready(function () { //main() 브라우저가 html코드를 다 읽고나면 여기 이벤트가 발생
        $('#removeBtn').on("click",function (){ //POST로 참조를 해야함
    ...
    $(document).ready(function () { //main() 브라우저가 html코드를 다 읽고나면 여기 이벤트가 발생
        $('#writeBtn').on("click",function (){ //POST로 참조를 해야함
            let form = $("#form");
    ...
    $(document).ready(function () { //main() 브라우저가 html코드를 다 읽고나면 여기 이벤트가 발생
        $('#modifyBtn').on("click",function (){ //POST로 참조를 해야함
            //1.  읽기 상태이면  수정 상태로 변경
            let form = $("#form"); //폼에 대한 참조를 얻어와서
            let isReadOnly = $("input[name=title]").attr('readonly'); // 리드온리인지 보기
            // input태그에 name=title인 요소의 애트리뷰트를 읽어온다.
